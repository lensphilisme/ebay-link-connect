...
export function RulesSettings() {
  ...
  const { data: rule, refetch } = useQuery({
  ...
  async function save(patch: any) {
    const { error } = await supabase.from("automation_rules").update(patch).eq("id", rule!.id);
    if (error) toast.error(error.message);
    else { toast.success("Rule saved"); refetch(); }
  }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Pricing & guardrails</CardTitle>
          <CardDescription>Applied when CJ products are pushed to eBay drafts.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex items-center justify-between rounded-lg border p-3 sm:col-span-2">
            <Label>Live Push to eBay enabled</Label>
            <Switch checked={rule.live_listing_enabled} onCheckedChange={(v) => save({ live_listing_enabled: v })} />
          </div>
          <div className="flex items-center justify-between rounded-lg border p-3 sm:col-span-2">
            <Label>Require preflight approval</Label>
            <Switch checked={rule.preflight_required} onCheckedChange={(v) => save({ preflight_required: v })} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">AI Optimizer thresholds</CardTitle>
          <CardDescription>Tune these, then Preview / Run on the Optimizer page.</CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex items-center justify-between rounded-lg border p-3 sm:col-span-2">
            <Label>End test listings after first sale</Label>
            <Switch checked={rule.end_test_listings_after_success} onCheckedChange={(v) => save({ end_test_listings_after_success: v })} />
          </div>
  ...
  return (
    <div className="space-y-1">
      ...
      <div className="flex gap-2">
        <Input type="number" step={step} value={local} onChange={(e) => setLocal(e.target.value)} />
        <Button variant="outline" onClick={() => onSave(Number(local))}>Save</Button>
      </div>
    </div>
  );
}
